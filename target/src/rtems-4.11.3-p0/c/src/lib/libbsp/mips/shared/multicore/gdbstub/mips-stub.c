...
*
*       Modified by Anton Ermakov, 2019
*
*******************************************************************************/
 ...
 */
    ...
    #define R_SZ 4
#elif (__mips == 32)        // Add mips32 support here
    typedef unsigned int mips_register_t;
    #define R_SZ 4
#else
...
void bsp_interrupt_catch( rtems_isr_entry, int, rtems_isr_entry *) ;
...
void mips_gdb_stub_install(int enableThreads)
{
   ...
   */
   ...
   for(i=0; exceptionVector[i] > -1; i++)
   {
      ...
      bsp_interrupt_catch( (rtems_isr_entry) handle_exception, exceptionVector[i], &old );
   }
