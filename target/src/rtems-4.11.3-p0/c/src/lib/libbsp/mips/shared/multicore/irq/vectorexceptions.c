...
typedef void (*bsp_isr_entry) (rtems_vector_number , CPU_Interrupt_frame *) ;

static bsp_isr_entry bsp_interrupt_vector[32] ;
...
void mips_vector_exceptions( CPU_Interrupt_frame *frame )
{
    ...
    const bsp_interrupt_handler_entry *e =
        &bsp_interrupt_handler_table [bsp_interrupt_handler_index(exc)];

    if (bsp_interrupt_vector[exc] != NULL) {
        bsp_interrupt_vector[exc](exc, frame) ;
        if (e != NULL) {  
...
/*
 *  Install exception handler which will be processed from BSP isr supervisor.
 */
rtems_status_code bsp_interrupt_catch( rtems_isr_entry handler, int vector, rtems_isr_entry *old_handler) {
    
    /* 
     * If vector is out of base MIPS exceptions, then
     * return an error.
     */
    ...
    old_handler = bsp_interrupt_vector[vector] ;
    bsp_interrupt_vector[vector] = handler ;
    
    DTRACEK("ISR: 0x%x handler is installed\n\r", vector) ;
